...
exp_name = 'HIV contact tracing analysis  (' + tracing_groups + ', sample rates: ' + \
 sample_rates + ', tracing rates: ' + tracing_rates + ', tracing start times: ' + \
 tracing_start_times + ', tracing end times: ' + tracing_end_times + ')'

# This is the actual command to be run on COMPS
workorder_str = f"""{{
    "Command": "singularity exec Assets/tree_stats_analysis.sif Rscript exp1.r",
    "NumCores": 1,
    "NumNodes": 1,
...
if 'exp_id' in locals():
    ...
    processing_exp.set_tags({'data_exp_id': data_exp.id, 'traceGroup': tracing_groups, 'sampleRate': sample_rates, 'traceRate': tracing_rates, 'traceStartTime': tracing_start_times, 'traceEndTime': tracing_end_times, 'tracingDelay': tracing_delays, 'lookBackWindow': look_back_durations, 'acuteToTrace': acute_to_traces})
    processing_exp.save()
...
for i, sim in enumerate(data_sims):
    ...
    proc_sim.set_tags({'uploadData_sim_id': sim.id, 'data_exp_id': data_exp.id, 'traceGroup': tracing_groups, 'sampleRate': sample_rates, 'traceRate': tracing_rates, 'traceStartTime': tracing_start_times, 'traceEndTime': tracing_end_times, 'tracingDelay': tracing_delays, 'lookBackWindow': look_back_durations, 'acuteToTrace': acute_to_traces, 'sim_num': i})
    ...
    proc_sim.add_file(SimulationFile('exp1.r', 'input'), file_path='exp1.r')
